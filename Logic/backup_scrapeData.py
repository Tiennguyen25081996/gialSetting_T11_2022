...
class ScrapDataWeb_Download_Gmail_bk:
    ...
    def Write_Log(self,pathfile,namefile,text):
        ...
        f = open(pathfile+date_time_Folder + namefile+"log.txt", "a", encoding='utf8')
        f.write(sentence +"\n")
        f.close()
    ...
    def search_email_by_date(self,driver,start_date,end_date):
        sql = ''.join(['after:',start_date,'before:',end_date])
        search_text = driver.find_elements(By.XPATH,"//*[@id='gs_lc50']/input[1]")[0]
        search_text.click()
        time.sleep(2)
        search_text.send_keys(sql)
        time.sleep(2)
        search_text.send_keys(Keys.ENTER)
 
    def check_map_link_download_HDDT(self,arr_title_HDDT_c,title_curent_email):
        check = 0
        for item in arr_title_HDDT_c:
            if re.search(item,title_curent_email):
                match item:
                    case 'tracuuhoadon.hc.com.vn':
                        self.Write_Log("./Output/",'/ThinkAndDo', 'mở web tracuuhoadon.hc.com.vn tải thành công')
                        check = 1
                        break 
                    case 'tracuu.ehoadon.vn':
                        self.Write_Log("./Output/",'/ThinkAndDo', 'mở web tracuu.ehoadon.vn tải thành công')
                        check = 2
                        break
                    case 'hd.easyinvoice.com.vn':
                        self.Write_Log("./Output/",'/ThinkAndDo', 'mở web hd.easyinvoice.com.vn tải thành công')
                        check = 3
                        break
                    case 'hddt.digiworld.com.vn':
                        self.Write_Log("./Output/",'/ThinkAndDo', 'mở web hddt.digiworld.com.vn tải thành công')
                        check = 4
                        break   
                    case 'www.xuathoadon.com.vn':
                        self.Write_Log("./Output/",'/ThinkAndDo', 'mở web www.xuathoadon.com.vn tải thành công')
                        check = 5
                        break  
                    case 'cadmin-group1.mifi.vn':
                        self.Write_Log("./Output/",'/ThinkAndDo', 'mở web cadmin-group1.mifi.vn tải thành công')
                        check = 6
                        break
                    case 'portaltool-mienbac.vnpt-invoice.com.vn':
                        self.Write_Log("./Output/",'/ThinkAndDo', 'mở web portaltool-mienbac.vnpt-invoice.com.vn tải thành công')
                        check = 7
                        break        
                    case default:#tracuu.smartcom.com.vn
                        self.Write_Log("./Output/",'/ThinkAndDo','[WARNING]Web hiện tại chưa training cho bot \n')
                        self.Write_Log("./Output/",'/ThinkAndDo','[WARNING]Web là : ' + str(item))
                        break
        return check
    ...
    def CreateFolderByTime(self,path):
        now = datetime.now() # current date and time
        date_time_Folder = now.strftime("%m%d%Y")
        print("Create Name Folder : ",date_time_Folder)
    ...
    def Auto_click_download_each_email(self,driver,arr_title_HDDT,arr_content_HDDT_c,arr_web_open_by_link,link_output,name_file_log_err,name_file_log_success,loop_all_page):
        ...
        for lp in range(0,loop_all_page):
            ...
            if lp != 0:
                print('Current Page : ',lp)
                self.click_next_page(driver,link_output,name_file_log_err)
                time.sleep(3)
                self.get_id_tr_span_arr(driver)
            print(len(self.arr_id_tr_span))
            print(self.arr_id_tr_span)
            for idx,item in enumerate(self.arr_id_tr_span):
                print('idx : ',idx)
                print('1st idx_email_select_in_page : ',idx_email_select_in_page)
                check_next_page = 0
                if (idx_email_select_in_page == idx):
                    ...
                    if check == 1:
                        ...
                        self.Write_Log(link_output,name_file_log_success, 'ID file attachment : '+ str(item))
                        time.sleep(2)
                        # check co file dinh kem
                        if len(self.arr_id_files_dl) > 0:
                            print(self.arr_id_files_dl)
                            self.Write_Log(link_output,name_file_log_success, 'Loai 1 : '+ str(len(self.arr_id_tr_span)))
                            time.sleep(1)
                            ...
                            self.Write_Log(link_output,name_file_log_success, 'Count gmail_gs_content :' + str(len(gmail_gs_content)))
                            if len(gmail_gs_content) > 0:
                                self.Write_Log(link_output,name_file_log_success, 'Have email content')
                                time.sleep(1)
                                link_text = self.get_link_download_file_pdf(driver,arr_content_HDDT_c,arr_title_HDDT)
                                if (link_text is not None):
                                    self.Write_Log(link_output,name_file_log_success, 'Tồn tại link download file PDF \n')
                                    self.Write_Log(link_output,name_file_log_success, 'Link download PDF : '+ str(link_text))
                                    time.sleep(1)
                                    check_link_text = self.check_map_link_download_HDDT(arr_web_open_by_link,link_text)
                                    if (check_link_text == 1):
                                        ...
                                        time.sleep(8)
                                        self.Write_Log(link_output,name_file_log_err, '[WARNING]Check Web download : ' + str(check_link_text))
                                        self.Write_Log(link_output,name_file_log_err, '[WARNING]Web hiện tại chưa training cho bot : \n')
                                        ...
                                        time.sleep(3)
                                    time.sleep(3)
                                    self.Write_Log(link_output,name_file_log_success, 'Link Download là : '+str(link_text))
